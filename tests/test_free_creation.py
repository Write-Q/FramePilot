...
def test_title_gets_written_before_review(tmp_path):
    model=Writer()
...
def test_repeated_problem_stops_loop(tmp_path):
    model=Writer([[issue()]])
    with StoryService(tmp_path,model) as service:
...
def test_free_can_repair_constraint_without_changing_user_requirement(tmp_path):
